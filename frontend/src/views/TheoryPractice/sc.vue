<template>
  <div>
      ...
      <div class="upfoot">
        ...
        <div class="fright">
          ...
          <span>
            ...
            {{ index + 1 }}/{{ collects.length }}
          </span>
        </div>
      </div>
      <ul class="upitem">
        <li
          v-for="(item, i) in collects"
          :key="i"
          class="item"
          :class="[
            { thisitem: index === i },
    ...
    <div class="handelBtns">
      ...
      <div
        class="next"
        :class="{ cantBtn: index >= collects.length - 1 }"
        @click="next"
      >
        下一题
      </div>
...
export default {
  created() {
    if (this.collects[this.index]) {
      fetch(`/g.asp?id=${this.collects[this.index]}`)
        .then((res) => {
          // console.log(res)
          return res.json();
        })
  ...
  methods: {
    ...
    collectSelect() {
      if (this.collect) {
        ...
        if (this.collects.length <= 1) {
          ...
          this.singlebtn(1);
        } else if (this.index >= this.collects.length - 1) {
          this.index = this.collects.length - 1;
          this.index--;
          this.fetchdata();
          // this.next()
          // if(this.index < this.collects.length - 1){
          //     this.index ++
          // }
    ...
    fetchdata() {
      this.show = false;
      let el = this.datas.find((item) => item.id === this.collects[this.index]);
      if (el) {
        // data: Object,
        fetch(`/g.asp?id=${this.collects[this.index]}`)
          // fetch(`/g.asp?id=1550`)
    ...
    singlebtn(i) {
      // console.log(i,this.collects.length )
