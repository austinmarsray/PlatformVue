<template>
  <div>
    ...
    <van-popup v-model="historyindexgo" round :style="{ width: '300px' }">
      <div class="begin_top">
        <div class="begin"></div>
        <div class="goon">上次练习到第{{ historyindex + 1 }}题,是否继续</div>
      </div>
      <div class="begin_buttom">
        <div class="cencel" @click="closehistoryindexgo">取消</div>
        <div class="certain" @click="gotohistoryindex">确定</div>
      </div>
    </van-popup>
...
<script>
import selectOption from "@/components/TheoryPractice/select-option";
...
export default {
  ...
  methods: {
    gotohistoryindex() {
      this.index = this.historyindex;
      this.historyindexgo = false;
      this.fetchdata();
      this.$emit("historyindex", 0);
    },
    closehistoryindexgo() {
      this.historyindexgo = false;
      this.$emit("historyindex", 0);
    ...
    clickupitem(i) {
      this.index = i;
      this.fetchdata();
      this.$emit("historyindex", this.index);
    },
