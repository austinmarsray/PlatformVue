<template>
  <div>
    ...
    >
      ...
      <ul class="upitem">
        <li
          v-for="(item, i) in wrongquestions"
          :key="i"
          class="item"
          :class="[
            { thisitem: index === i },
    ...
    <div class="handelBtns">
      ...
      <div
        class="next"
        :class="{ cantBtn: index >= wrongquestions.length - 1 }"
        @click="next"
      >
        下一题
      </div>
    ...
    <footer>
      <div class="fleft" @click="wrongquestionsdetach">
        <span>移除</span>
      </div>
      <div class="fright">
        <span>
...
<script>
import selectOption from '@/components/TheoryPractice/select-option.vue'

export default {
  created() {
    if (this.wrongquestions[this.index]) {
      fetch(`/g.asp?id=${this.wrongquestions[this.index]}`)
        .then((res) => {
          // console.log(res)
          return res.json();
        })
  ...
  methods: {
    ...
    wrongquestionsdetach() {
      if (this.wrongquestions.length > 0) {
        this.$emit("wrongquestionsdetach", this.data.id);

        if (this.wrongquestions.length <= 1) {
          this.show = false;
          this.data = {};
          this.index = -1;
          this.select = true;
          ...
          this.singlebtn(1);
        } else if (this.index >= this.wrongquestions.length - 1) {
          this.index = this.wrongquestions.length - 1;
          this.index--;
          this.fetchdata();
        } else {
          this.next();
    ...
    fetchdata() {
      this.show = false;
      let el = this.datas.find(
        (item) => item.id === this.wrongquestions[this.index]
      );
      if (el) {
        // data: Object,
        fetch(`/g.asp?id=${this.wrongquestions[this.index]}`)
    ...
    singlebtn(i) {
      if (i > this.wrongquestions.length - 1) {
    ...
    next() {
      if (this.index < this.wrongquestions.length - 1) {
        this.index++;
        this.fetchdata();
      } else {
        this.singlebtnshow = true;
